...
- 2026-09-20 20:34 CEST: Implemented Revision 3. Multi question order uses seed `1000 + k` and rotates by `k mod n` after each shuffle. Missing option mass uses the last returned token probability, capped at 0.01, before normalization; coverage still uses real mass. Single mode keeps the floor disabled and its tests unchanged. Set `MULTI_FORMAT = 3` and tested rejection of revision 2 cache entries. Validation with `UV_CACHE_DIR=/tmp/system1-uv-cache`: `uv run ruff format . && uv run ruff check . && uv run pytest -q` passed. Ruff reported `1 file reformatted, 17 files left unchanged` and `All checks passed!`; pytest reported `128 passed, 1 skipped in 0.81s`. Live tests skipped in the sandbox. No server or benchmark was run. No commit was made.
